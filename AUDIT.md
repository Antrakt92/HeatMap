...
- Sensor parsing и UI update logic покрыты только точечно; не хватает вариантов имен sensors, fan/VRAM cases и desktop-state transitions.
...
1. Расширить fake LHM tests на GPU/VRAM/fan parsing и варианты sensor names.
2. Добавить DLL manifest и verification command для всего `lib/`.
3. Определиться с architecture policy в `setup.py`: поддержать x86/ARM64 или явно ограничить x64.
...
- Расширить fake LHM tests: GPU/VRAM/fan parsing, варианты sensor names и peek/UI degraded states.
