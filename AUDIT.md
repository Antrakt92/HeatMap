...
- Есть `unittest` для helper/startup/error-state логики, autostart command/result handling, runtime DLL verification, no-LHM fallback, partial sensor failures, LHM init sanity-check и базовых peek toggle transitions, но desktop-state transitions и большая часть tkinter/WinAPI поведения остаются без тестов.
...
- Добавить tests на desktop/peek visibility transitions и компактные UI degraded states.
...
2. Расширить fake LHM tests на варианты sensor names и desktop/peek/UI transitions.
...
- Расширить fake LHM tests: варианты sensor names и desktop/peek/UI degraded states.
