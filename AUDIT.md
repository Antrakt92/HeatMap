...
- Clean install сейчас сломан: `python setup.py` не восстанавливает зависимости из пустой папки.
...
- Указать точные реальные пути в NuGet-пакете, например `runtimes/win-x64/lib/net472/LibreHardwareMonitorLib.dll`.
- Исправить `HidSharp` URL на `2.6.4` или заменить ожидаемый hash на реальный hash `2.1.0`.
- Лучше: отказаться от basename fallback для DLL с несколькими target frameworks; target framework и architecture должны быть явным выбором.
- Добавить smoke test для `setup.py`, который запускает установку в temporary directory.
...
- После `join(timeout=5)` проверять `self.sensor_thread.is_alive()`.
- Если поток жив, не закрывать `self.computer` из UI thread; логировать warning и дать daemon завершиться, либо ввести отдельный lock/lifecycle state, который гарантирует отсутствие активного `read_sensors()`.
- Рассмотреть более короткие/безопасные timeouts внутри hardware reads, если LHM API позволяет.
...
- Для числовых полей явно отклонять bool: `isinstance(value, bool)` до проверки `int/float`.
- Добавить тесты config normalization на `true`, `false`, строки, `null`, отрицательные и слишком большие значения.
...
- Использовать `math.isfinite(v)` вместо ручной проверки только NaN.
- Добавить тесты `_safe_round(None)`, `NaN`, `inf`, `-inf`, нормальных float/int значений.
...
- Удалить поле или начать использовать его осознанно, если нужен cached HWND.
...
1. Починить clean install: исправить `setup.py` paths/versions/hashes и добавить smoke test во временной папке.
2. Починить P1 UI/alert mismatch: синхронизировать color thresholds с alert thresholds, особенно disk temp.
3. Добавить file logging и user-visible error path для autostart/LHM/config/sensor failures.
4. Вынести pure logic из `overlay.py` и добавить первые `pytest` тесты:
   - thresholds/colors;
   - config normalization;
   - sensor parsing на fake sensors;
   - autostart command construction.
5. Исправить shutdown race вокруг `sensor_thread.join(timeout=5)` и `self.computer.Close()`.
6. Сделать атомарную запись `overlay_config.json`.
7. Ужесточить config validation: bool не должен проходить как int.
8. Добавить `math.isfinite()` в `_safe_round()`.
9. Добавить DLL manifest и verification command для всего `lib/`.
10. Улучшить position clamp с учетом размера виджета и multi-monitor changes.
11. Удалить мертвые `HWND_TOPMOST/HWND_NOTOPMOST` и `_hwnd`.
12. Добавить `LICENSE`.
...
- Срочно починить `setup.py`: реальные NuGet paths, версия `HidSharp`, hashes и smoke test clean install во временной папке.
...
- Исправить shutdown race: не закрывать LHM object, если sensor thread не завершился после timeout.
- Перевести сохранение конфига на atomic write с backup поврежденного JSON.
- Ужесточить config validation, чтобы `true/false` не проходили как числа.
- Защитить `_safe_round()` от `inf/-inf`.
- Описать и проверять все DLL из `lib/` через manifest с SHA256.
- Добавить clamp позиции виджета по фактическому размеру окна.
- Удалить мертвые topmost constants и `_hwnd`.
