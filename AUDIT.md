...
При последующем отчёте о зависании Peek после клика подтверждено, что native Tk
wrapper возвращал MA_ACTIVATE несмотря на WS_EX_NOACTIVATE. Теперь обработчик
WM_MOUSEACTIVATE оставляет приложение под ним активным, сохраняя доставку клика.
Регрессия на настоящем HWND падает до исправления и проходит после. Добавить
к игровому acceptance клик по показаниям/заголовку и уход курсора без клика в игре;
отдельно проверить меню и ввод в диалоге CPU reference.
Пользователь повторил зависание без клика после этой правки. Свежая runtime-запись
показала завершившийся slide-out и `peek=False`: проблема была в повторном показе
независимого desktop fallback, а не в остановке таймера. Теперь полное перекрытие
его места окном приложения удерживает fallback скрытым и cloaked. Проверены
повторный Peek, Show Desktop, исключения для свёрнутых/cloaked/собственных окон;
новая native-регрессия падает до исправления и проходит после. Точный игровой
повтор после этой правки остаётся частью acceptance, а не следствием числа тестов.
