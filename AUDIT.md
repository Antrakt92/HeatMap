...
- Ошибки runtime в `pythonw.exe` почти не видны пользователю; sensor-loop errors дополнительно оставляют на экране старые значения без явного degraded/error state.
- Стартовый порядок все еще рискованный: новый процесс сначала убивает старый instance и только потом проверяет критические prerequisites.
- Single-instance защита покрывает documented `run_as_admin.bat`, но может пропустить ручной запуск `python overlay.py` из README.
...
- Перенести проверку критических prerequisites до `kill_previous_instances()`.
- Либо сделать двухфазный старт: сначала убедиться, что новый процесс способен инициализироваться достаточно далеко, и только потом завершать старый instance.
- После изменения добавить regression test для порядка startup checks через мок `kill_previous_instances()`.
...
- Ввести явный UI state для sensor errors: `ERR`, серый цвет, tooltip/status label или compact degraded indicator.
- Хранить timestamp последнего успешного sample и показывать stale state после заданного TTL.
- Добавить тест на `update_ui()`/view-model слой: error sample не должен оставлять last-good values как будто они свежие.
...
- Нормализовать candidate argv values: `strip`, `abspath`, `normcase`, проверка basename `overlay.py`.
- Для relative args использовать `proc.cwd()` как базу, если доступно.
- Добавить тестируемый helper `is_same_script_invocation(script_path, argv, cwd)` и покрыть absolute/relative/quoted cases.
...
1. Исправить startup ordering: prerequisites до завершения старого instance.
2. Сделать sensor-loop failures видимыми в UI и не оставлять stale values как свежие.
3. Улучшить single-instance matching для relative/manual launches.
4. Добавить file logging и user-visible error path для autostart/LHM/config/sensor failures.
5. Расширить тестовый слой:
   - sensor parsing на fake LHM objects;
   - autostart command/result handling;
   - больше config edge cases.
6. Добавить DLL manifest и verification command для всего `lib/`.
7. Определиться с architecture policy в `setup.py`: поддержать x86/ARM64 или явно ограничить x64.
...
- Исправить startup ordering: проверять критические DLL/prerequisites до завершения старого instance.
- Сделать sensor-loop failures видимыми в UI: error/degraded state, stale timestamp, без показа старых значений как свежих.
- Улучшить single-instance matching для ручного `python overlay.py`, relative paths, quoted paths и cwd-based normalization.
