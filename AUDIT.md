...
## P2 - Проверяемое управление общим контроллером SYS4/SYS5/SYS6

Пользователь подтвердил отсутствие водяного охлаждения: Pump в названии LHM —
тип разъёма, а не обнаруженная помпа. Тем не менее перехват SYS4 через текущую
библиотеку меняет общий EC; ошибки возврата EC могут быть проигнорированы, а
совпадение PWM не подтверждает восстановление его автоматического режима.
Полный набор ограничений и проверка формул: `docs/airflow-audit-2026-09-06.md`.

SYS4/SYS5/SYS6 пока остаются плате. Для расширения управления нужны проверка
фактического EC-режима и исходных битов выходов, отсутствие промежуточной нулевой
команды при перехвате и успешное восстановление всех затронутых каналов. Смена DLL
или прямые MMIO/EC-записи в этом аудите не выполнялись. Схема подключения и
эффективность совместного обдува под реальной нагрузкой остаются непроверенными.
Проверки независимых SYS1/SYS2 перед перехватом и после возврата, а также запуск
обновлённого контроллера на реальном ПК подтверждены для 1.2.0-rc.3:
`docs/startup-controller-1.2.0-rc.3.md`. Это не закрывает ограничения общего EC.
